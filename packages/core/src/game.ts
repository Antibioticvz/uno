import { createDeck, drawTopCard, RandomFn, shuffle } from './deck.js'
import { Card, GameError, GameState, PlayerId, Result, RoomId } from './types.js'
...
const replenishDrawPile = (state: GameState, random: RandomFn): Result<void> => {
...
export const listPlayableCards = (state: GameState, playerId: PlayerId): Result<Card[]> => {
  ...
  const playable =
    state.players[playerIndex]?.hand.filter((card) =>
...
export const getPlayerHand = (state: GameState, playerId: PlayerId): Result<Card[]> => {
  const playerIndex = findPlayerIndex(state, playerId)
  if (playerIndex === -1) {
    return err({ code: 'PLAYER_ALREADY_JOINED', message: 'Игрок отсутствует в комнате.' })
  }
